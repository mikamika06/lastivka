    ...
    <div className="flex items-center gap-2.5">
      <span className="hidden items-center gap-2 sm:flex">
        ...
        <span className="text-xs">
          <span className="font-semibold text-ink">{t(session.name)}</span>
          <span className="text-faint"> · {t(session.scopeLabel)}</span>
        </span>
      ...
      <button
        onClick={logout}
        className="rounded-lg border border-line bg-surface px-2 py-1 text-xs font-medium text-muted transition hover:text-ink"
      >
